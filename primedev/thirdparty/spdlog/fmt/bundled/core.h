...
#if defined(__has_include) && !defined(__INTELLISENSE__) && (!FMT_ICC_VERSION || FMT_ICC_VERSION >= 1600)
...
#ifndef FMT_OVERRIDE
#if FMT_HAS_FEATURE(cxx_override_control) || (FMT_GCC_VERSION >= 408 && FMT_HAS_GXX_CXX11) || FMT_MSC_VER >= 1900
...
namespace detail
{
	...
	template <typename T> constexpr T const_check(T value)
	{
		return value;
	}
...
public:
	...
	constexpr basic_string_view(const Char* s, size_t count) FMT_NOEXCEPT : data_(s), size_(count) {}
...
/**
  ...
  **Example**::
  ...
  \endrst
 */
template <typename Char, FMT_ENABLE_IF(is_char<Char>::value)> inline basic_string_view<Char> to_string_view(const Char* s)
{
	return s;
}
...
template <typename Char> inline basic_string_view<Char> to_string_view(basic_string_view<Char> s)
{
	return s;
}
...
namespace detail
{
	...
	template <typename..., typename S, FMT_ENABLE_IF(!is_compile_string<S>::value)> FMT_INLINE void check_format_string(const S&)
	{
...
/**
  ...
  \endrst
 */
template <typename Char, typename ErrorHandler = detail::error_handler> class basic_format_parse_context : private ErrorHandler
{
private:
...
public:
	...
	explicit constexpr basic_format_parse_context(basic_string_view<Char> format_str, ErrorHandler eh = {}, int next_arg_id = 0)
		: ErrorHandler(eh)
...
namespace detail
{
	...
	protected:
		...
		buffer(T* p = nullptr, size_t sz = 0, size_t cap = 0) FMT_NOEXCEPT : ptr_(p), size_(sz), capacity_(cap) {}
	...
	public:
		...
		template <typename I> const T& operator[](I index) const { return ptr_[index]; }
	};
	...
	template <typename OutputIt, typename T, typename Traits = buffer_traits> class iterator_buffer final : public Traits, public buffer<T>
	{
	private:
		OutputIt out_;
		enum
	...
	template <typename Container> class iterator_buffer<
	...
	public:
		...
		std::back_insert_iterator<Container> out() { return std::back_inserter(container_); }
	};
	...
	template <typename T> class buffer_appender : public std::back_insert_iterator<buffer<T>>
	{
	...
	template <typename T, typename OutputIt> iterator_buffer<OutputIt, T> get_buffer(OutputIt);
	...
	template <typename Buffer> auto get_iterator(Buffer& buf) -> decltype(buf.out())
	{
		return buf.out();
	}
	...
	template <typename T, typename Context> using has_fallback_formatter =
	...
	template <typename T, typename Char, size_t NUM_ARGS, size_t NUM_NAMED_ARGS> struct arg_data
	{
		...
		template <typename... U> arg_data(const U&... init)
	...
	template <typename T, typename Char, size_t NUM_ARGS> struct arg_data<T, Char, NUM_ARGS, 0>
	{
		...
		template <typename... U> FMT_INLINE arg_data(const U&... init)
			: args_ {init...}
		{
		}
	...
	template <typename Char, typename T, typename... Tail> void init_named_args(
		named_arg_info<Char>* named_args, int arg_count, int named_arg_count, const named_arg<Char, T>& arg, const Tail&... args)
	{
		named_args[named_arg_count++] = {arg.name, arg_count};
		init_named_args(named_args, arg_count + 1, named_arg_count, args...);
	}

	template <typename... Args> FMT_INLINE void init_named_args(std::nullptr_t, int, int, const Args&...) {}
	...
	template <bool B = false> constexpr size_t count()
	{
		return B ? 1 : 0;
	}
...
#define FMT_TYPE_CONSTANT(Type, constant)                                                                                                  \
	...
	template <typename Context> struct arg_mapper
	{
		...
		template <typename T, FMT_ENABLE_IF(is_string<T>::value)> FMT_CONSTEXPR basic_string_view<char_type> map(const T& val)
		{
		...
		template <typename T> FMT_CONSTEXPR auto map(const named_arg<char_type, T>& val)
...
private:
	...
	template <typename ContextType, typename T> friend FMT_CONSTEXPR basic_format_arg<ContextType> detail::make_arg(const T& value);

	template <typename Visitor, typename Ctx> friend FMT_CONSTEXPR auto visit_format_arg(Visitor&& vis, const basic_format_arg<Ctx>& arg)
		-> decltype(vis(0));
...
public:
	...
	constexpr explicit operator bool() const FMT_NOEXCEPT { return type_ != detail::type::none_type; }
...
namespace detail
{
	...
	template <typename It, typename T> struct is_output_iterator<
		It,
		T,
		void_t<typename std::iterator_traits<It>::iterator_category, decltype(*std::declval<It>() = std::declval<T>())>> : std::true_type
	{
	};

	template <typename OutputIt> struct is_back_insert_iterator : std::false_type
	{
	};
	...
	template <typename Container> struct is_contiguous_back_insert_iterator<std::back_insert_iterator<Container>> : is_contiguous<Container>
	{
	};
	template <typename Char> struct is_contiguous_back_insert_iterator<buffer_appender<Char>> : std::true_type
	{
	};
	...
	template <typename> constexpr unsigned long long encode_types()
	{
		return 0;
	}
	...
	template <bool IS_PACKED, typename Context, type, typename T, FMT_ENABLE_IF(IS_PACKED)> inline value<Context> make_arg(const T& val)
	{
...
private:
	...
	static constexpr unsigned long long desc = (is_packed ? detail::encode_types<Context, Args...>() : detail::is_unpacked_bit | num_args) |
...
public:
	format_arg_store(const Args&... args)
		...
		,
#endif
		data_ {detail::make_arg<is_packed, Context, detail::mapped_type_constant<Args, Context>::value>(args)...}
	{
...
inline auto make_args_checked(const S& format_str, const remove_reference_t<Args>&... args)
	-> format_arg_store<buffer_context<Char>, remove_reference_t<Args>...>
{
	static_assert(
		detail::count<(std::is_base_of<detail::view, remove_reference_t<Args>>::value && std::is_reference<Args>::value)...>() == 0,
...
/**
  ...
  **Example**::
  ...
  \endrst
 */
template <typename Char, typename T> inline detail::named_arg<Char, T> arg(const Char* name, const T& arg)
{
...
private:
	...
	template <typename T> using stored_type = conditional_t<detail::is_string<T>::value, std::basic_string<char_type>, T>;
...
public:
	...
	*/
	...
	{
		static_assert(
			detail::is_named_arg<typename std::remove_cv<T>::type>::value || need_copy<T>::value,
...
private:
	...
	bool has_named_args() const { return (desc_ & detail::has_named_args_bit) != 0; }
...
public:
	...
	/**
	 ...
	 \endrst
	 */
	template <typename... Args> FMT_INLINE basic_format_args(const format_arg_store<Context, Args...>& store)
...
{
	template <typename... Args> FMT_INLINE format_args(const Args&... args)
...
namespace detail
{
	...
	template <typename Char> void vformat_to(
		buffer<Char>& buf,
...
inline auto format_to(OutputIt out, const S& format_str, Args&&... args) -> typename std::enable_if<enable, OutputIt>::type
{
...
vformat_to_n(OutputIt out, size_t n, basic_string_view<Char> format_str, basic_format_args<buffer_context<type_identity_t<Char>>> args)
{
 ...
 */
template <typename OutputIt, typename S, typename... Args, bool enable = detail::is_output_iterator<OutputIt, char_t<S>>::value>
inline auto format_to_n(OutputIt out, size_t n, const S& format_str, const Args&... args) ->
 ...
 */
template <typename... Args> inline size_t formatted_size(string_view format_str, Args&&... args)
{
	const auto& vargs = fmt::make_args_checked<Args...>(format_str, args...);
...
/**
  ...
  **Example**::
  ...
  \endrst
 */
template <typename S, typename... Args, typename Char = char_t<S>> inline void print(std::FILE* f, const S& format_str, Args&&... args)
{
	const auto& vargs = fmt::make_args_checked<Args...>(format_str, args...);
	return detail::is_unicode<Char>() ? vprint(f, to_string_view(format_str), vargs)
...
/**
  ...
  **Example**::
  ...
  \endrst
 */
template <typename S, typename... Args, typename Char = char_t<S>> inline void print(const S& format_str, Args&&... args)
{
	const auto& vargs = fmt::make_args_checked<Args...>(format_str, args...);
	return detail::is_unicode<Char>() ? vprint(to_string_view(format_str), vargs)
