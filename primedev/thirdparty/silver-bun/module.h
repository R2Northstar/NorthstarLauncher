...
public:
	struct ModuleSections_t
	{
		...
		ModuleSections_t(const char* sectionName, uintptr_t pSectionBase, size_t nSectionSize)
			...
			, m_pSectionBase(pSectionBase)
			, m_nSectionSize(nSectionSize)
		{
		}
...
private:
	CMemory FindPatternSIMD(
		const uint8_t* pPattern, const char* szMask, const ModuleSections_t* moduleSection = nullptr, const size_t nOccurrence = 0) const;
