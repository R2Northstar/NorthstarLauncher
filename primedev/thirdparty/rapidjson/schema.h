			...
			if (const ValueType* v = GetMember(value, GetPatternPropertiesString()))
			{
				...
				for (ConstMemberIterator itr = v->MemberBegin(); itr != v->MemberEnd(); ++itr)
				{
					...
					schemaDocument->CreateSchema(
						&patternProperties_[patternPropertyCount_].schema, q.Append(itr->name, allocator_), itr->value, document);
					patternPropertyCount_++;
				}
			}
				...
				for (ConstMemberIterator itr = dependencies->MemberBegin(); itr != dependencies->MemberEnd(); ++itr)
				{
					...
					if (FindPropertyIndex(itr->name, &sourceIndex))
					{
						...
						else if (itr->value.IsObject())
						{
							...
							schemaDocument->CreateSchema(
								&properties_[sourceIndex].dependenciesSchema, q.Append(itr->name, allocator_), itr->value, document);
...
#define RAPIDJSON_STRING_(name, ...)                                                                                                       \
	...
	{                                                                                                                                      \
		...
		RAPIDJSON_STRING_(
			AdditionalProperties, 'a', 'd', 'd', 'i', 't', 'i', 'o', 'n', 'a', 'l', 'P', 'r', 'o', 'p', 'e', 'r', 't', 'i', 'e', 's')
...
private:
	...
	{
		for (const SchemaEntry* target = schemaMap_.template Bottom<SchemaEntry>(); target != schemaMap_.template End<SchemaEntry>();
			 ++target)
...
public:
	...
	*/
	...
	{
		return documentStack_.Empty() ? PointerType()
...
#undef RAPIDJSON_SCHEMA_HANDLE_VALUE_
	...
	virtual void* CreateHasher() { return new (GetStateAllocator().Malloc(sizeof(HasherType))) HasherType(&GetStateAllocator()); }
