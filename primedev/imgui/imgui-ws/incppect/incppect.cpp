...
struct Incppect::Impl
{
	...
	void run()
	{
		...
		uWS::TemplatedApp<false>(uWS::SocketContextOptions{

			}).ws<PerSocketData>(
			   "/incppect",
			   uWS::App::WebSocketBehavior<PerSocketData>{
				   .compression = uWS::CompressOptions(uWS::SHARED_COMPRESSOR)
				   .maxPayloadLength = parameters.maxPayloadLength_bytes
				   .idleTimeout = parameters.tIdleTimeout_s
	...
	void update()
	{
		for (auto& [clientId, cd] : clientData)
		{
			...
			{
				my_printf(
					"[incppect] warning: buffered amount = %d, not sending updates to client %d. waiting for buffer to drain\n",
			...
			for (auto& [requestId, req] : cd.requests)
			{
					...
					tCur - req.tLastUpdated_ms > req.tMinUpdate_ms)
				{
					...
					{
						std::copy(
							(char*)(&dataSize_bytes), (char*)(&dataSize_bytes) + sizeof(dataSize_bytes), std::back_inserter(curBuffer));
			...
			if (curBuffer.size() > 4)
			{
				if (curBuffer.size() == prevBuffer.size() && curBuffer.size() > 256)
				{
					...
					{
						my_printf(
							"[incppect] warning: buffer size (%d) exceeds maxPayloadLength (%d)\n",
							(int)diffBuffer.size(),
	...
	: m_impl(new Impl())
{
	...
	var("incppect.ip_address[%d]",
		[this](const TIdxs& idxs)
		{
